...
ls -lt $BASE/$DIR_FILES/*.txt | awk 'BEGIN{printf "debug -o debug.txt\n" > "'$BASE_PI'/ftp_script.ftp"; printf "open '$IP'\n" >> "'$BASE_PI'/ftp_script.ftp"; printf "user '$USER' \"'$PASS'\"\n" >> "'$BASE_PI'/ftp_script.ftp";printf "cd gravimetro2\n" >> "'$BASE_PI/ftp_script.ftp"}{printf "put %s\n",$NF >> "'$BASE_PI'/ftp_script.ftp"} END{printf "bye\n" >> "'$BASE_PI'/ftp_script.ftp"}' | bash
